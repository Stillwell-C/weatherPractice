export const setPlaceHolderText = () => {
    ...
    window.innerWidth < 400 ? (input.placeholder = "City, State, Country") : (input.placeholder = "City, State, Country, or US Zip Code")
}
...
const updateWeatherLocationHeader = (message) => {
    ...
    if (message.indexOf('Lat:') !== -1 && message.indexOf('Long:') !== -1) {
        ...
        const lat = mapArray[1].indexOf('-') === -1 ? mapArray[1].slice(0, 5) : mapArray[1].slice(0, 6)
        const lon = mapArray[3].indexOf('-') === -1 ? mapArray[3].slice(0, 5) : mapArray[3].slice(0, 6)
...
const clearDisplay = () => {
    const currentConditons = document.getElementById('currentForecast__conditions');
    deleteContents(currentConditons)
